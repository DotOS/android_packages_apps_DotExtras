...
public class AccentPicker extends InstrumentedDialogFragment implements OnClickListener {
    ...
    private void initView() {
        ...
        if (mView != null) {
            ...
            // Change the accent picker button depending on whether or not the dark, black, Neo theme is applied
            blackAccent.setBackgroundColor(getResources().getColor(
                    (isUsingDarkTheme() || isUsingBlackTheme() || isUsingNeoTheme()) ? R.color.accent_picker_white_accent : R.color.accent_picker_dark_accent));
    ...
    // Check for the neo theme overlay
    private boolean isUsingNeoTheme() {
        OverlayInfo themeInfo = null;
        try {
